...
It uses the **TcpServerBase.connStatusCluster.ctl** with the `connStatus` boolean, and the `message` string. 
If all the TCP clients connect to the TCP servers, a `GenRead` user event is triggered with the boolean TRUE.
If one of the TCP clients disconnects, a `GenRead` user event is triggered with the boolean FALSE.

The user has to check whether the `message` is an empty string or not.
If it is empty, the boolean value can be used to know the connection status.
Otherwise, the boolean value should not be used.
...
| timeoutServerInMs | Time in ms that the `TCP Wait on Listener` function waits for a TCP connection. This will be used in stopServer occurrence timeout and in the runServer Event Structure timeout, too. |
