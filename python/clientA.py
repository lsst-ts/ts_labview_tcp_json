...
async def main():
    ...
    print(f'Connect to the server A.')
    await asyncio.gather(
        tcp_client_cmd.connect(localhost, 50000),
        tcp_client_tel.connect(localhost, 50001),
    )
    ...
    # Wait for an event sent by the component simulator A
    print(f'You have 10 seconds to send an event from the Component Simulator A.')
    await tcp_client_cmd.read(timeout=10.0) 
     
    # Wait for telemetry sent by the component simulator A
    print(f'You have 10 seconds to send telemetry from the Component Simulator A.')
    await tcp_client_tel.read(timeout=10.0) 
    
    # Publish multiple telemetry to the component simulator A
    print(f'Publishing multiple telemetry to the Component Simulator A.')
    ...
    print(f'Disconnect from the server A.')
    await asyncio.gather(
